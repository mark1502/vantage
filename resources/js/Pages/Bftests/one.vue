...
const arr1 = ref([ 5, 10, 15, 20, 25, 30 ]);
const arr2 = ref([]);
...
function encodeA( num ) {
    arr2.value.push(num);
}

function encodeB( num ) {
    arr2.value.push(num);
}

function encodeC( num ) {
    arr2.value.push(num);
}

function encode() {
    for( let i=0; i < 6; i++ ) {
        const thenum = arr1.value[i];
        const arrPosition = i % 3;
        switch( arrPosition ) {
            case 0:
                encodeA( thenum );
                break;
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="overflow-hidden bg-white shadow-xs sm:rounded-lg">
                    ...
                    <div class="flex flex-col ml-5">
                        <div v-for="number in arr1" :key="number" class="" >{{ number }}</div>
                    </div>
                    <div class="flex flex-col ml-5">
                        <div v-for="number in arr2" :key="number" class="" >{{ number }}</div>
                    </div>
